...
- **OBS 断流自动恢复**：若 OBS 出现“反复重连后掉线”，脚本不仅会在非活跃宽限期后恢复，还会统计 reconnect 抖动次数；在窗口内达到阈值会主动重设推流并重启流（带冷却与次数限制），尽量避免直播被动中断。
...
- 若 B 站返回人脸认证（例如 code `60024`），脚本会直接标记错误并停止该阶段重试，避免无意义连环重试；需要人工完成后再继续。
